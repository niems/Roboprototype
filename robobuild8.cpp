...
//keeps all kinematic bodies between the two boundaries
void kinematicBoundaries(sf::Vector2f &left_boundary, sf::Vector2f &right_boundary, vector<Object*> &body);
    ...
    while (window.isOpen())
    {
		...
		if(in_focus == true)
		{
			...
			if(game_state == GAME_STATE::LIVE) //if you're in live mode
			{
				...
				kinematicBoundaries(left_boundary, right_boundary, editor.getKinematicObjects());
			}
