...
enum STATIC{SMALLEST_PLATFORM, SMALL_PLATFORM, LARGE_PLATFORM}; 
enum DYNAMIC{CRATE, CRATE2, SPHERE, SPHERE2}; 
...
int main()
{
	...
	editor_object_type.setPosition( game_mode_text.getPosition().x, game_mode_text.getPosition().y + editor_object_type.getGlobalBounds().height + 20.0);
	...
	mouse_pos.y = window.getSize().y / 1.0;
	...
	if(!small_platform_texture.loadFromFile("images//stone_ledge.png") )
	{
	...
	mouse_pos.x = editor.getStaticTextures()[editor.getCurrentIndex()].getSize().x / 2.0;
	mouse_pos.y = window.getSize().y / 1.2;
	editor.createStaticBody(window, world, mouse_pos);

	
	//create kinematic platform
	editor.setCurrentIndex(KINEMATIC::MOVING_PLATFORM);
	mouse_pos.x = editor.getKinematicTextures()[editor.getCurrentIndex()].getSize().x * 1.5;
	mouse_pos.y = window.getSize().y / 1.4 * -1.0;
	editor.createKinematicBody(window, world, mouse_pos);
	editor.getKinematicObjects()[editor.getCurrentIndex()]->getBody()->SetLinearVelocity( b2Vec2(200.0 * PIXELS_TO_METERS, 0.0) );
	sf::Vector2f left_boundary(mouse_pos.x, mouse_pos.y);
	sf::Vector2f right_boundary( (window.getSize().x - editor.getKinematicTextures()[editor.getCurrentIndex()].getSize().x / 2.0) - 100, mouse_pos.y );
	cout << "kinematic: " << editor.getKinematicObjects().size() << endl;
	...
	temp_object->getBody()->SetTransform( b2Vec2( (small_platform_texture.getSize().x * 1.5) * PIXELS_TO_METERS, -(window.getSize().y / 1.4) * PIXELS_TO_METERS), 0.0 );
	...
	sf::Vector2f right_boundary( (window.getSize().x - small_platform_texture.getSize().x / 2.0) - 100, temp_object->getSprite()->getPosition().y );
    ...
    while (window.isOpen())
    {
		...
		if(in_focus == true)
		{
			...
			if(game_state == GAME_STATE::LIVE) //if you're in live mode
			{
				...
				kinematicBoundaries(editor.getKinematicObjects().back(), left_boundary, right_boundary); 
			}
			...
			else if(game_state == GAME_STATE::EDITOR) //if you're in editor mode
			{
				...
				editor.keyboardActionCommands(window, world, mouse_clock, editor_object_type, mouse_pos, editor_type); 
			...
			{
				Draw::drawEditor(window, editor, mouse_pos, editor_object_type, game_state);
			}
